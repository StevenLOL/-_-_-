...
OEVENTLUA[30057] = function(aa, bb) --洗武功
	local head = math.random(350, 575)
	Cls()
	say("糟糕....好像迷路了。啊，怎么又下起雨来了，怎么办....")	
	Cls()
	say("前面好像有个山洞，不管了，先进去避一避再说。")		
	Cls()
	say("这位少侠....", head, 0, "？？？")
	Cls()
	say("啊！吓我一跳....老前辈，你住在这里？")	
	Cls()
	say("我在此隐居多年，还从来没有人找到过这里。你我有缘，我传你一些武功如何？", head, 0, "？？？")	
	Cls()
	say("真的假的....山谷里的奇遇？我难道真的是主角？")	
	if DrawStrBoxYesNo(-1, -1, "要接受么？", C_WHITE, 30) == false then	
		Cls()
		say("多谢前辈，只是晚辈以为，武学之道贵精不贵多，前辈的好意晚辈心领了。")	
		Cls()
	...
	while hasit == 1 do
		...
		for i = 1, HHH_GAME_SETTING["WG_COUNT_MAX"] do
			if JY.Person[zj()]["武功"..i] > 0 and JY.Person[zj()]["武功"..i] == wugong then
				hasit = 1			
			end
		end
		count = count + 1
	...
	JY.Person[zj()]["武功1"] = wugong
	JY.Person[zj()]["武功等级1"] = 900 --math.random(750, 900) 武骧金星：取消这个没有意义的设置
	Cls()
	dark()
	light()
	QZXS(JY.Person[zj()]["姓名"].."领悟"..JY.Wugong[wugong]["名称"].."！")
	say("如何？看懂了吗？", head, 0, "？？？")	
	Cls()
	say("懂了。谢谢前辈指点。")	
	Cls()
